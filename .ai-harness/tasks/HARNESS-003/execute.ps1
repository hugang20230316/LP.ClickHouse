...
if ([string]$task.execution_mode -ne 'agent') {
    throw "HARNESS-003 execute 仅支持 execution_mode=agent，当前值：$($task.execution_mode)"
}
...
if ($provider -ne 'codex') {
    throw "HARNESS-003 v1 仅支持 codex provider，当前值：$provider"
}
...
Write-Host "HARNESS-003 execute completed. Provider=$provider Model=$model Attempts=$($result.attempt_count)"
